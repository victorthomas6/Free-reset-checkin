...
function parseEntries(txt){
 ...
 blocks.forEach(b=>{
  const head=b.match(/FR2\s*\|\s*(?:(Sprint|Challenge|Relay|Marathon)\s*\|\s*)?([A-Za-z]{1,4})\s*\|\s*D\s*(\d+)(?:\s*\/\s*(\d+))?(?:\s*\|\s*(\d{4}-\d{2}-\d{2}|—))?/i);if(!head)return;
  ...
  rows.push({program:p.name,totalDays:p.days,id:head[2].toUpperCase(),day:+head[3],date:head[5]&&head[5]!=='—'?head[5]:null,fast:f?span(f[1],f[2]):null,close:f?f[1]:null,sleep:r?span(r[1],r[2]):null,bed:r?r[1]:null,meals:e?+e[1]:null,fruit:e?e[2].toUpperCase():null,graze:e?e[3].toUpperCase():null,walks:x?+x[1]:null,mins:x?+x[2]:null,sun:x?x[3].toUpperCase():null,en:s?+s[1]:null,cl:s?+s[2]:null,sq:s?+s[3]:null,hu:s?+s[4]:null,wt:m?num(m[1]):null,waist:m?num(m[2]):null,bp:m&&m[3]?m[3].trim():null,ok:!!(head&&f&&r&&e&&x&&s)});
 });
...
function readPaste(){
 ...
 if(!rows.length){box.innerHTML='<p class="bad">No check-ins recognised. New entries start with <code>FR2 | Challenge | BO | D3/10 | 2026-08-17</code>. Older <code>FR2 | BO | D3</code> Relay entries still work.</p>';return;}
 ...
 sum+='</tbody></table></div>';
 let det='<h3 style="margin-top:22px">Every entry</h3><div class="scroll"><table><thead><tr><th>Program</th><th>ID</th><th>Date</th><th>Day</th><th>Fast</th><th>Close</th><th>Sleep</th><th>Bed</th><th>Meals</th><th>Fruit</th><th>Graze</th><th>Walks</th><th>Min</th><th>Sun</th><th>E</th><th>C</th><th>S</th><th>H</th><th>Wt</th><th>Waist</th></tr></thead><tbody>';
 rows.forEach(r=>{det+=`<tr><td>${r.program}</td><td class="id">${r.id}</td><td>${r.date??'—'}</td><td>${r.day}/${r.totalDays}</td><td>${hm(r.fast)}</td><td>${pretty(r.close)}</td><td>${hm(r.sleep)}</td><td>${pretty(r.bed)}</td><td>${r.meals??'—'}</td><td>${r.fruit??'—'}</td><td>${r.graze??'—'}</td><td>${r.walks??'—'}</td><td>${r.mins??'—'}</td><td>${r.sun??'—'}</td><td>${r.en??'—'}</td><td>${r.cl??'—'}</td><td>${r.sq??'—'}</td><td>${r.hu??'—'}</td><td>${r.wt??'—'}</td><td>${r.waist??'—'}</td></tr>`;});det+='</tbody></table></div>';
