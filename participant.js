...
function syncProgram(){
 ...
 $('day').max=String(p.days); $('dayOf').textContent=`of ${p.days}`;
 const raw=Number(val('day'))||1,clamped=Math.max(1,Math.min(p.days,raw)); if(raw!==clamped)$('day').value=String(clamped);
...
function build(){
 ...
 let out=`FR2 | ${p.name} | ${(val('pid')||'??').toUpperCase()} | D${day}\n`;
...
['pid','day','program','lastBite','firstBite','bed','wake','wt','waist','bp','note'].forEach(id=>$(id).addEventListener('input',build));
